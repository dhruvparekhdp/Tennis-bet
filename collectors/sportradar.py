...
class SportradarCollector:
    ...
    async def fetch_tennis(self, api_key: str) -> None:
        """Fetch live summaries + today's schedule so upcoming matches show on the dashboard."""
        ...
        async with httpx.AsyncClient(timeout=20.0) as client:
            ...
            # ── 2. Today's schedule (upcoming matches) ─────────────────────────
            try:
                today = datetime.utcnow().strftime("%Y-%m-%d")
                sched_url = _TENNIS_SCHEDULE_URL.format(date=today)
                resp2 = await client.get(sched_url, headers=_header(api_key))
                if resp2.status_code == 200:
                    for sport_event in resp2.json().get("sport_events", []):
                        try:
                            state = await self._parse_tennis_scheduled(sport_event)
                            if state and state.match_id not in live_ids:
                                await self.tennis_store.update(state)
                                live_ids.add(state.match_id)
                        except Exception:
                            log.exception("sportradar_sched_parse_error",
                                          event=sport_event.get("id", "?"))
                    log.info("sportradar_schedule_fetched",
                             date=today, count=resp2.json().get("sport_events", []).__len__())
                else:
                    log.debug("sportradar_schedule_status", status=resp2.status_code)
            except Exception:
                log.exception("sportradar_tennis_schedule_failed")
